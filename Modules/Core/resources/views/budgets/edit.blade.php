...
@php
    $budgetIsRecurring = $budget->is_recurring ?? true;
    $periodStartValue = $budget->period_start ? $budget->period_start->format('Y-m') : date('Y-m');
@endphp
<div class="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-8" x-data="{
    ...
    isRecurring: '{{ old('is_recurring', $budgetIsRecurring ? '1' : '0') }}',
    formatCurrency() {
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <label class="cursor-pointer group">
                            <input type="radio" name="is_recurring" value="1" class="peer sr-only" {{ old('is_recurring', $budgetIsRecurring ? '1' : '0') == '1' ? 'checked' : '' }} x-model="isRecurring">
                        ...
                        <label class="cursor-pointer group">
                            <input type="radio" name="is_recurring" value="0" class="peer sr-only" {{ old('is_recurring', $budgetIsRecurring ? '1' : '0') == '0' ? 'checked' : '' }} x-model="isRecurring">
                            <div class="p-4 flex flex-col items-center justify-center rounded-2xl bg-gray-50 dark:bg-gray-950 border-2 border-gray-200 dark:border-white/10 peer-checked:border-emerald-500 peer-checked:bg-emerald-500/5 transition-all group-hover:bg-gray-100 dark:group-hover:bg-white/5">
                    ...
                    <div x-show="isRecurring === '0' || isRecurring === 0" x-cloak class="mt-4">
                        ...
                        <input type="month" name="period_start" id="period_start" value="{{ old('period_start', $periodStartValue) }}"
