...
<div style="display: flex; gap: 12px; margin-bottom: 24px; flex-wrap: wrap;">
    ...
    <div style="flex: 1; min-width: 120px; padding: 12px 16px; background: #fef2f2; border: 1px solid #fecaca; border-radius: 8px;">
        ...
        <div style="font-size: 18px; font-weight: 800; color: #b91c1c;">{{ format_currency($summary['expense'] ?? 0) }}</div>
    </div>
    <div style="flex: 1; min-width: 120px; padding: 12px 16px; background: #eff6ff; border: 1px solid #bfdbfe; border-radius: 8px;">
        ...
        <div style="font-size: 18px; font-weight: 800; color: #1d4ed8;">{{ format_percent($summary['savings_rate'] ?? 0, 1) }}</div>
    </div>
</div>
@endif
...
<table>
    ...
    <tbody>
        ...
        <tr class="item-row">
            ...
            <td class="text-right">{{ format_currency($item['total'] ?? 0) }}</td>
        </tr>
        @endforeach
        ...
        <tr class="total-row">
            ...
            <td class="text-right">{{ format_currency($ranking->sum('total')) }}</td>
        </tr>
        @endif
    </tbody>
</table>
