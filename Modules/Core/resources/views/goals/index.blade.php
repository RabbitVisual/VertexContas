<x-paneluser::layouts.master>
        ...
        <div>
            <h2 class="font-black text-3xl text-slate-800 dark:text-white flex items-center">
                <div class="bg-primary/10 dark:bg-primary/20 p-2 rounded-xl mr-3">
                    <x-icon name="bullseye" class="text-primary" />
                </div>
                Minhas Metas
            </h2>
        ...
        @can('create', \Modules\Core\Models\Goal::class)
            <a href="{{ route('core.goals.create') }}"
               class="group relative inline-flex items-center justify-center px-6 py-3 text-base font-bold text-white transition-all duration-200 bg-primary font-pj rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary hover:bg-primary-dark shadow-lg shadow-primary/30">
                <x-icon name="plus" class="mr-2" /> Nova Meta
            </a>
        @endcan
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($goals as $goal)
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
                <!-- Header -->
                <div class="flex justify-between items-start mb-6">
                    <div class="flex items-center gap-3">
                        <div class="w-12 h-12 rounded-xl bg-{{ $statusColor }}-50 dark:bg-{{ $statusColor }}-900/10 flex items-center justify-center text-{{ $statusColor }}-600 dark:text-{{ $statusColor }}-400">
                             <x-icon name="{{ $percentage >= 100 ? 'trophy' : 'bullseye' }}" class="text-xl" />
                        </div>
                        <div>
                            ...
                            @if($goal->deadline)
                                <div class="flex items-center text-xs text-slate-500 dark:text-slate-400">
                                    <x-icon name="clock" class="mr-1 text-[10px]" />
                                    <span>
                ...
                <div class="mb-6">
                    ...
                    <div class="relative w-full bg-slate-100 dark:bg-slate-700 rounded-full h-4 overflow-hidden">
                        <div class="absolute top-0 left-0 h-full bg-gradient-to-r from-{{ $statusColor }}-500 to-{{ $statusColor }}-400 rounded-full transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(59,130,246,0.5)]"
                    ...
                    <div class="flex justify-between items-center mt-2 text-xs">
                        <span class="font-bold text-{{ $statusColor }}-600 dark:text-{{ $statusColor }}-400 bg-{{ $statusColor }}-50 dark:bg-{{ $statusColor }}-900/20 px-2 py-0.5 rounded-md">
                            {{ number_format($percentage, 0) }}% Concluído
                        </span>
                        @if($remaining > 0)
                            <span class="text-slate-400">Falta R$ {{ number_format($remaining, 2, ',', '.') }}</span>
                        @else
                            <span class="text-emerald-500 font-bold flex items-center"><x-icon name="check" class="mr-1" /> Conquista Desbloqueada!</span>
                        @endif
                    </div>
                </div>
        ...
        @empty
             <div class="col-span-full py-16 text-center">
                <div class="inline-flex items-center justify-center w-24 h-24 rounded-full bg-slate-100 dark:bg-slate-800 mb-6 group-hover:scale-110 transition-transform">
                    <x-icon name="bullseye" class="text-4xl text-slate-300 dark:text-slate-600" />
                </div>
