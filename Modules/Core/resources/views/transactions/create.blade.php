    ...
    <div class="space-y-8 pb-8" x-data="{
            ...
            <div class="relative p-6 md:p-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                <div class="flex-1">
                    <div class="inline-flex items-center gap-2 px-4 py-2 bg-primary-500/20 border border-primary-400/30 rounded-full backdrop-blur-md mb-4">
                        <x-icon name="plus-circle" class="w-4 h-4 text-primary-300" />
                        <span class="text-primary-200 text-xs font-black uppercase tracking-[0.2em]">Movimentação</span>
                    </div>
                    <h1 class="text-3xl md:text-4xl lg:text-5xl font-black text-white tracking-tight leading-tight">Nova Transação</h1>
                    <p class="text-slate-400 font-medium max-w-xl mt-2 text-base leading-relaxed">Registre suas movimentações financeiras no detalhe</p>
                </div>
        ...
        <div class="max-w-4xl mx-auto">
            <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                <form action="{{ route('core.transactions.store') }}" method="POST">
                    @csrf
                        ...
                        <div class="lg:col-span-12 p-8 lg:p-12 space-y-10">
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <button type="button"
                                            @click="type = 'income'"
                                            :class="type === 'income' ? 'bg-emerald-600 border-emerald-600 text-white shadow-lg shadow-emerald-900/20' : 'bg-slate-50 dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800'"
                                    ...
                                    <button type="button"
                                            @click="type = 'expense'"
                                            :class="type === 'expense' ? 'bg-rose-600 border-rose-600 text-white shadow-lg shadow-rose-900/20' : 'bg-slate-50 dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800'"
                                            class="flex flex-col items-center gap-3 p-6 rounded-3xl border-2 transition-all group">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="relative">
                                        ...
                                        <select name="status" id="status" class="w-full pl-11 pr-4 py-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 focus:border-primary-500 focus:ring-4 focus:ring-primary-500/5 transition-all appearance-none font-bold text-slate-700 dark:text-slate-200" required>
                                            <option value="completed" {{ old('status', 'completed') == 'completed' ? 'selected' : '' }}>Concluída / Paga</option>
                                            <option value="pending" {{ old('status') == 'pending' ? 'selected' : '' }}>Pendente / Agendada</option>
                                        </select>
                                        <div class="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400">
                                            <x-icon name="chevron-down" size="xs" />
                                        </div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="relative">
                                    <textarea name="description" id="description" rows="3" class="w-full p-6 bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-3xl focus:border-primary-500 focus:ring-4 focus:ring-primary-500/5 transition-all font-bold text-slate-700 dark:text-slate-200 placeholder-slate-400" placeholder="Digite uma descrição para identificar esta transação...">{{ old('description') }}</textarea>
                                </div>
                                @error('description')
                                    <p class="mt-1 text-xs text-rose-500 font-bold ml-1">{{ $message }}</p>
                                @enderror
                            ...
                            @if($isPro)
                                <div class="pt-6 grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div class="p-6 bg-amber-50 dark:bg-amber-900/10 rounded-3xl border border-amber-200 dark:border-amber-900/30">
                                        ...
                                        <p class="text-[11px] text-amber-800 dark:text-amber-500/80 leading-relaxed">Habilite para criar esta transação automaticamente todos os meses.</p>
                                    </div>
                                    ...
                                    <div class="p-6 bg-primary-50 dark:bg-primary-900/10 rounded-3xl border border-primary-200 dark:border-primary-900/30">
                                        ...
                                        <div class="flex items-center justify-center p-4 border-2 border-dashed border-primary-200 dark:border-primary-800 rounded-2xl group cursor-pointer hover:bg-white dark:hover:bg-slate-900 transition-all">
                                            <span class="text-[10px] font-black uppercase tracking-widest text-primary-600 dark:text-primary-400 group-hover:scale-110 transition-transform">Adicionar Comprovante</span>
                                        </div>
                                    </div>
                                </div>
                            @endif
