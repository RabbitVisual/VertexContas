            ...
            <div class="relative p-6 md:p-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                <div class="flex-1">
                    <div class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-500/20 border border-emerald-400/30 rounded-full backdrop-blur-md mb-4">
                        <x-icon name="plus-circle" style="duotone" class="w-4 h-4 text-emerald-300" />
                        <span class="text-emerald-200 text-xs font-black uppercase tracking-[0.2em]">Nova conta</span>
                    </div>
                    ...
                    <p class="text-slate-400 font-medium max-w-xl mt-2 text-base md:text-lg leading-relaxed">
                        Preencha os dados para criar sua conta. O nome aparecerá no cartão.
                    </p>
                </div>
        ...
        <div class="max-w-2xl">
            <div class="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                <div class="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
                    ...
                    <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Preencha os campos para criar sua conta</p>
                </div>
                <form action="{{ route('core.accounts.store') }}" method="POST" class="p-6 lg:p-8 space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}"
                               class="bg-slate-50 dark:bg-slate-700/50 border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white text-sm rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 block w-full p-3.5 transition-colors @error('name') border-red-500 @enderror"
                               placeholder="Ex: Nubank, Cartão Pessoal, Carteira João" required>
                        <p class="mt-1.5 text-xs text-slate-500 dark:text-slate-400">Esse nome será exibido no cartão da conta</p>
                        @error('name')
                    ...
                    <div>
                        <label for="balance" class="block text-sm font-medium text-slate-900 dark:text-white mb-2">Saldo inicial *</label>
                        <div class="relative">
                        ...
                        <p class="mt-1.5 text-xs text-slate-500 dark:text-slate-400">Informe o saldo atual desta conta</p>
                        @error('balance')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400 flex items-center gap-1"><x-icon name="circle-exclamation" style="solid" class="w-4 h-4" /> {{ $message }}</p>
                        @enderror
                    </div>
