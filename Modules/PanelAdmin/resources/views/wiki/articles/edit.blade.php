<x-paneladmin::layouts.master>
    <x-slot name="navbarTitle">Editar Artigo Wiki</x-slot>

    <x-paneladmin::page title="Editar Artigo Wiki" :subtitle="Str::limit($article->title, 50)">
        @if(session('error'))
        ...
        <form action="{{ route('admin.wiki.articles.update', $article) }}" method="POST" class="max-w-4xl space-y-6">
            @csrf
            @method('PUT')
            <x-paneladmin::card title="Conteúdo" subtitle="Título, categoria e corpo do artigo (Markdown suportado).">
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="title" required value="{{ old('title', $article->title) }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-medium focus:ring-2 focus:ring-[#11C76F]/20 focus:border-[#11C76F]">
                        </div>
                        <div class="space-y-2">
                            <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Categoria</label>
                            <div class="relative">
                                <select name="category_id" required class="wiki-select w-full pl-4 pr-10 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-medium focus:ring-2 focus:ring-[#11C76F]/20 focus:border-[#11C76F]">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="content" rows="14" required class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-medium text-sm resize-y focus:ring-2 focus:ring-[#11C76F]/20 focus:border-[#11C76F]">{{ old('content', $article->content) }}</textarea>
                    </div>
                    <div class="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700">
                        <div>
                            <span class="text-sm font-bold text-slate-900 dark:text-white block">Publicado</span>
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('admin.wiki.articles') }}" class="px-6 py-3 rounded-xl border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-400 font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">Cancelar</a>
                <button type="submit" class="px-6 py-3 bg-[#11C76F] hover:bg-[#0EA85A] text-white font-bold rounded-xl transition-colors flex items-center gap-2">
                    <x-icon name="floppy-disk" style="duotone" class="w-5 h-5" /> Atualizar Artigo
                </button>
            </div>
        </form>
    </x-paneladmin::page>
