<x-paneladmin::layouts.master>
    ...
    <x-paneladmin::page title="Usuários do Sistema" subtitle="Gerencie contas, planos e permissões.">
        <x-paneladmin::card>
            <x-slot name="header">
            ...
            <x-paneladmin::table-wrapper>
                <x-slot name="thead">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Plano/Role</th>
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors even:bg-slate-50/50 dark:even:bg-slate-800/30">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                @if($user->photo)
                                    <img src="{{ $user->photo_url }}" alt="" class="h-9 w-9 rounded-xl object-cover shrink-0 border border-slate-200 dark:border-slate-600">
                                @else
