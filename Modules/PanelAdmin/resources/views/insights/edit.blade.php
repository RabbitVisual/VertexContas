<x-paneladmin::layouts.master>
    <x-slot name="navbarTitle">Editar Dica</x-slot>
    ...
    <div class="max-w-3xl mx-auto space-y-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-black text-slate-800 dark:text-white tracking-tight">Editar Dica</h1>
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1">Atualize a mensagem do assistente virtual.</p>
            </div>
        </div>

        <form action="{{ route('admin.insights.update', $insight) }}" method="POST" class="bg-white dark:bg-slate-900 rounded-[2rem] p-8 border border-gray-100 dark:border-gray-800 shadow-sm space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-2">
                ...
                <select name="trigger_event" id="trigger_event" required class="w-full px-6 py-4 bg-gray-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-[#11C76F]/20 text-slate-800 dark:text-white font-medium text-sm">
                    <option value="low_balance" {{ old('trigger_event', $insight->trigger_event) === 'low_balance' ? 'selected' : '' }}>Saldo baixo</option>
                    <option value="budget_reached" {{ old('trigger_event', $insight->trigger_event) === 'budget_reached' ? 'selected' : '' }}>Orçamento atingido</option>
                    <option value="savings_milestone" {{ old('trigger_event', $insight->trigger_event) === 'savings_milestone' ? 'selected' : '' }}>Marco de economia</option>
                    <option value="daily_tip" {{ old('trigger_event', $insight->trigger_event) === 'daily_tip' ? 'selected' : '' }}>Dica do dia</option>
                </select>
            ...
            <div class="space-y-2">
                ...
                <textarea name="content" id="content" rows="4" required class="w-full px-6 py-4 bg-gray-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-[#11C76F]/20 text-slate-800 dark:text-white font-medium text-sm resize-none">{{ old('content', $insight->content) }}</textarea>
            ...
            <div class="space-y-2">
                ...
                <select name="level" id="level" required class="w-full px-6 py-4 bg-gray-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-[#11C76F]/20 text-slate-800 dark:text-white font-medium text-sm">
                    <option value="info" {{ old('level', $insight->level) === 'info' ? 'selected' : '' }}>Info</option>
                    <option value="success" {{ old('level', $insight->level) === 'success' ? 'selected' : '' }}>Sucesso</option>
                    <option value="warning" {{ old('level', $insight->level) === 'warning' ? 'selected' : '' }}>Aviso</option>
                    <option value="danger" {{ old('level', $insight->level) === 'danger' ? 'selected' : '' }}>Perigo</option>
                </select>
            ...
            <div class="flex items-center gap-4 p-4 bg-gray-50 dark:bg-slate-800 rounded-2xl">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $insight->is_active) ? 'checked' : '' }} class="rounded border-slate-300 dark:border-slate-600 text-[#11C76F] focus:ring-[#11C76F]/20">
                <label for="is_active" class="text-sm font-bold text-slate-700 dark:text-slate-300">Ativo (visível para o robô)</label>
            </div>

            <div class="flex items-center gap-4 p-4 bg-gray-50 dark:bg-slate-800 rounded-2xl">
                <input type="checkbox" name="is_pro_only" id="is_pro_only" value="1" {{ old('is_pro_only', $insight->is_pro_only) ? 'checked' : '' }} class="rounded border-slate-300 dark:border-slate-600 text-[#11C76F] focus:ring-[#11C76F]/20">
