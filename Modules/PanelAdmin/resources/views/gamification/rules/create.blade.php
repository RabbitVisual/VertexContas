<x-paneladmin::layouts.master>
    <x-slot name="navbarTitle">Nova Regra</x-slot>
    ...
    <div class="max-w-3xl mx-auto space-y-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-black text-slate-800 dark:text-white tracking-tight">Nova Regra de Conquista</h1>
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1">Configure a condição para desbloquear uma medalha.</p>
            </div>
        </div>

        <form action="{{ route('admin.gamification.rules.store') }}" method="POST" class="bg-white dark:bg-slate-900 rounded-[2rem] p-8 border border-gray-100 dark:border-gray-800 shadow-sm space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <select name="condition_type" id="condition_type" required class="w-full px-6 py-4 bg-gray-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-[#11C76F]/20 text-slate-800 dark:text-white font-medium text-sm">
                        <option value="pillar_threshold" {{ old('condition_type') === 'pillar_threshold' ? 'selected' : '' }}>Pilar 50/30/20 (essential, lifestyle, financial)</option>
                        <option value="reserve_months" {{ old('condition_type') === 'reserve_months' ? 'selected' : '' }}>Reserva (meses)</option>
                        <option value="consecutive_days" {{ old('condition_type') === 'consecutive_days' ? 'selected' : '' }}>Dias consecutivos</option>
                        <option value="savings_threshold" {{ old('condition_type') === 'savings_threshold' ? 'selected' : '' }}>Poupança %</option>
                        <option value="pro_subscription" {{ old('condition_type') === 'pro_subscription' ? 'selected' : '' }}>Assinante PRO</option>
                    </select>
                ...
                <div class="space-y-2 md:col-span-2">
                    ...
                    <select name="medal_id" id="medal_id" class="w-full px-6 py-4 bg-gray-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-[#11C76F]/20 text-slate-800 dark:text-white font-medium text-sm">
                        ...
                        @foreach($medals as $medal)
                            <option value="{{ $medal->id }}" {{ old('medal_id') == $medal->id ? 'selected' : '' }}>{{ $medal->title }} ({{ $medal->trigger_key }})</option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" name="priority" id="priority" required min="0" max="999" value="{{ old('priority', 80) }}" class="w-full px-6 py-4 bg-gray-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-[#11C76F]/20 text-slate-800 dark:text-white font-medium text-sm">
                    <p class="text-[10px] text-slate-400">Maior = avaliado primeiro</p>
                    @error('priority') <p class="text-sm text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>

                <div class="space-y-2">
                    <label for="level" class="block text-xs font-black text-slate-500 uppercase tracking-widest ml-1">Nível (Vertex Bot)</label>
                    <select name="level" id="level" required class="w-full px-6 py-4 bg-gray-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-[#11C76F]/20 text-slate-800 dark:text-white font-medium text-sm">
                        ...
                        <option value="success" {{ old('level', 'success') === 'success' ? 'selected' : '' }}>Sucesso</option>
            ...
            <div class="space-y-2">
                <label for="condition_params" class="block text-xs font-black text-slate-500 uppercase tracking-widest ml-1">Parâmetros JSON (condition_params)</label>
                <textarea name="condition_params" id="condition_params" rows="6" placeholder='{"operator": ">=", "value": 3, "min_monthly_expenses": 500}' class="w-full px-6 py-4 bg-gray-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-[#11C76F]/20 text-slate-800 dark:text-white font-mono text-xs resize-none">{{ old('condition_params', '{}') }}</textarea>
                <p class="text-[10px] text-slate-400">Ex: reserve_months: {"operator":"&gt;=","value":3,"min_monthly_expenses":500}. Consecutive: {"operator":"&gt;=","value":5,"min_transaction_count":15}. pillar: {"pillar":"essential","operator":"&lt;=","value":50,"min_baseline_income":500}</p>
            ...
            <div class="space-y-2">
                ...
                <textarea name="message_override" id="message_override" rows="2" class="w-full px-6 py-4 bg-gray-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-[#11C76F]/20 text-slate-800 dark:text-white font-medium text-sm resize-none">{{ old('message_override') }}</textarea>
