<x-paneluser::layouts.master :title="__('Dashboard')">
    <!-- Financial Overview Cards -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <!-- Balance Card -->
        <div class="relative overflow-hidden bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 group hover:shadow-md transition-all">
            <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                <i class="fa-solid fa-wallet text-6xl text-primary-500"></i>
            </div>
            ...
            <div class="mt-4 flex items-center text-xs">
                <span class="flex items-center {{ ($balance ?? 0) >= 0 ? 'text-emerald-500' : 'text-rose-500' }} font-bold bg-gray-50 dark:bg-gray-700/50 px-2 py-1 rounded-lg">
                    <i class="fa-solid {{ ($balance ?? 0) >= 0 ? 'fa-arrow-trend-up' : 'fa-arrow-trend-down' }} mr-1"></i>
                    {{ __('Total Available') }}
                </span>
            </div>
        </div>
        ...
        <div class="relative overflow-hidden bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 group hover:shadow-md transition-all">
             <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                <i class="fa-solid fa-arrow-down text-6xl text-emerald-500"></i>
            </div>
        ...
        <div class="relative overflow-hidden bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 group hover:shadow-md transition-all">
             <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                <i class="fa-solid fa-arrow-up text-6xl text-rose-500"></i>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
             <div class="flex justify-between items-center mb-6">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white flex items-center">
                    <i class="fa-solid fa-chart-area mr-2 text-primary-500"></i>
                    {{ __('Cash Flow') }}
                </h3>
             </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
            <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-6 flex items-center">
                <i class="fa-solid fa-chart-pie mr-2 text-primary-500"></i>
                {{ __('Expenses by Category') }}
            </h3>
            <div id="spendingChart" class="w-full min-h-[280px] flex items-center justify-center">
                @if($spendingByCategory->isEmpty())
                     <div class="text-center py-8 opacity-50">
                        <i class="fa-solid fa-chart-pie text-4xl text-gray-300 mb-2"></i>
                        <p class="text-sm font-bold text-gray-400">{{ __('No expenses data yet') }}</p>
                    </div>
                @endif
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="relative pl-4 border-l border-gray-200 dark:border-gray-700 space-y-8">
                        ...
                        <div class="flex justify-between items-start pl-4 hover:translate-x-1 transition-transform duration-200">
                            <div>
                                ...
                                <div class="flex items-center gap-2 mt-1">
                                    <span class="text-xs font-semibold px-2 py-0.5 rounded {{ $transaction->type === 'income' ? 'text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 dark:text-emerald-400' : 'text-rose-600 bg-rose-50 dark:bg-rose-900/20 dark:text-rose-400' }}">
                                        {{ $transaction->category->name ?? __('General') }}
                                    </span>
                ...
                @empty
                    <div class="text-center py-12 pl-4">
                        <i class="fa-solid fa-ghost text-4xl text-gray-300 dark:text-gray-600 mb-2"></i>
                        <p class="text-sm font-bold text-gray-400">{{ __('No recent transactions') }}</p>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
            <div class="flex justify-between items-center mb-6">
                 <h3 class="text-lg font-bold text-gray-900 dark:text-white flex items-center">
                    <i class="fa-solid fa-bullseye mr-2 text-primary-500"></i>
                    {{ __('Goals') }}
                </h3>
            ...
            <div class="space-y-6">
                        ...
                        <div class="flex justify-between items-end mb-2">
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                    {{ __('Remaining') }} <span class="font-semibold text-gray-700 dark:text-gray-300">R$ {{ number_format($remaining, 2, ',', '.') }}</span>
                                </p>
                            </div>
                ...
                @empty
                    <div class="text-center py-8">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Define goals to track your progress') }}</p>
                    </div>
                @endforelse
            </div>
             ...
             @if($goals->count() > 0)
                <div class="mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">
                    <a href="{{ route('core.goals.index') ?? '#' }}" class="block w-full text-center py-2 text-sm text-gray-600 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-500 bg-gray-50 dark:bg-gray-900/50 rounded-lg transition-colors font-medium">
                        {{ __('View all goals') }}
                    </a>
                </div>
            @endif
